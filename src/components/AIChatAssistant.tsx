...
const AIChatAssistant: React.FC = () => {
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Limit context to prevent token overflow, just basic info
            const shopsContext = shops.slice(0, 5).map(s => `- ${s.name}: ${s.address} (Open: ${s.hours})`).join('\n');
            const servicesContext = services.slice(0, 10).map(s => `- ${s.name} (${s.type})`).join('\n');
            ...
            setMessages(prev => [...prev, { id: Date.now() + 1, text: "Service temporarily unavailable. Please try again later.", sender: 'bot' }]);
        } finally {
            setIsLoading(false);
        }
    };
